        ...
        bool search(FT* x, FT& v) override {
            ...
            */
            ...
            auto step = [&] () {
                ...
                if (mOptions.mSearchType == SearchTypes::ANNEALING)
                {
                    FT xtmp[n];
                    FT fn; 
                    for (int i = 0; i < mOptions.maxStepNumber; i++) 
                    {
                        for (int j = 0; j < n; j++) 
                        {
                        ...
                        if ((fn < fcur) || ((rand() / (double)RAND_MAX) <= pow(e, -(fn - fcur) / annealing_temp)))
                        {
